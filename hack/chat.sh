#!/usr/bin/env bash
# Interactive chat with a deployed Agent Plane agent.
#
...
#   defaults: namespace=agent-plane-demo agent=support-agent
set -uo pipefail
...
kubectl -n agent-plane-system port-forward svc/agent-plane-registry 9090:9090 >/tmp/agent-plane-chat-reg.log 2>&1 &
REG_PID=$!
...
if kubectl -n "$NS" get svc orders-mcp >/dev/null 2>&1; then
  kubectl -n "$NS" port-forward svc/orders-mcp 18080:8080 >/tmp/agent-plane-chat-mcp.log 2>&1 &
  MCP_PID=$!
fi
