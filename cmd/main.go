...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "8cd146dd.hkmdxlftjf.io",
		// Step down voluntarily on shutdown so a rollout's outgoing pod exits
		// cleanly instead of failing lease renewal mid-termination ("leader
		// election lost" → pod Error). Safe here: main returns immediately
	...
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupToolWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Tool")
			os.Exit(1)
		}
	}
