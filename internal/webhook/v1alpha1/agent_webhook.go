...
func SetupAgentWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).For(&corev1alpha1.Agent{}).
...
// +kubebuilder:webhook:path=/validate-core-hkmdxlftjf-io-v1alpha1-agent,mutating=false,failurePolicy=fail,sideEffects=None,groups=core.hkmdxlftjf.io,resources=agents,verbs=create;update,versions=v1alpha1,name=vagent-v1alpha1.kb.io,admissionReviewVersions=v1
...
func (v *AgentCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	return nil, agent.Spec.Validate()
}

// ValidateUpdate implements webhook.CustomValidator so a webhook will be registered for the type Agent.
func (v *AgentCustomValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	agent, ok := newObj.(*corev1alpha1.Agent)
	if !ok {
