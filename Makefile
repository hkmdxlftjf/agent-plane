...
.PHONY: chart-crds
chart-crds: ## Sync the Helm chart's CRD templates with config/crd/bases.
	hack/chart-crds.sh

.PHONY: chart-crds-check
chart-crds-check: ## Fail if the Helm chart's CRD templates are out of date.
	hack/chart-crds.sh --check
...
KIND_CLUSTER ?= agent-plane-test-e2e
...
build-registry: fmt vet ## Build the Registry (data-plane config endpoint) binary.
	go build -o bin/registry ./cmd/registry

.PHONY: run-registry
run-registry: ## Run the Registry against the current kubeconfig context.
	go run ./cmd/registry
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name agent-plane-builder
	$(CONTAINER_TOOL) buildx use agent-plane-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm agent-plane-builder
	rm Dockerfile.cross
